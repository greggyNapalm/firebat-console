...
``['step', rps_from, rps_to, ste_val, 'step_dur']``
    Ступенчатая нагрука - последовательность постоянных нагрузок разной интенсивности. ``rps_from`` - стартовое, ``rps_to`` - конечно значения нагрузки. ``ste_val`` - величина шага в rps  c которым мы будим двигаться, ``step_dur`` - длительность каждой ступени в секундах. Если разность rps_to - rps_from меньше размера ступени, будет отстреляна только первая ступень.
